...
test_case.check_fatal_violation() once before and once after the step
runs - so a fatal Rulebook violation is noticed at step boundaries even
if the step never polls internally. This doesn't catch a violation
mid-step for a step that runs long without polling on its own - see
testcases/asimov/live_rulebook_runner.py's docstring for why, and for
test_case.wait_for() to close that gap yourself in a long-running step.

The first positional argument must have .set_state(), .test_id, and
.check_fatal_violation() - i.e. a TestCase.
"""
