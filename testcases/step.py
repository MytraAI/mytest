...
test_case.check_fatal_violation()/check_stop_requested()/
check_recording_alive() once before and once after the step runs - so a
fatal Rulebook violation, an external stop request (see
...
.check_fatal_violation(), .check_stop_requested(), and
.check_recording_alive() - i.e. a TestCase.
"""
...
def step(func: F) -> F:
    ...
    def wrapper(test_case, *args, **kwargs):
        ...
        test_case.check_recording_alive()
